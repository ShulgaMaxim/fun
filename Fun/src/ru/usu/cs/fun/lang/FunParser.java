...
public class FunParser extends AbstractLL1Parser {
	...
	public FunParser(FunScope aScope) {
		...
		add("Statement", new String[] { "let", "name", ":=", "Term", ";" },
				new ParseAction() {
		...
		add("Term", new String[] { "float", "int", "string", "name" },
				new String[] { "Atom", "Tail" }, new ParseAction() {
					public Terms execute(Object[] items) {
		...
		add("Term", new String[] { "fun", "(", "name", ")", "Term" },
				new ParseAction() {
					public Terms execute(Object[] items) {
						Term body = ((Terms) items[4]).build();
						String name = ((Lexeme) items[2]).getText();
						Abstraction abstraction = new Abstraction(name, name
								.startsWith("~"), body);
		...
		// Tail ::= // return null;
		add("Tail", new String[] { ";", ")", }, new String[] {},
				new ParseAction() {
					public Terms execute(Object[] items) {
						return null;
					}
		...
		add("Atom", new String[] { "int" }, new ParseAction() {
			public FunInt execute(Object[] items) {
				Object value = ((Lexeme) items[0]).getValue();
				return new FunInt((Integer) value);
			}
		});

		// Atom ::= string
		add("Atom", new String[] { "string" }, new ParseAction() {
			public FunString execute(Object[] items) {
				Object value = ((Lexeme) items[0]).getValue();
				return new FunString(value.toString());
			}
		});
		...
		// Atom ::= float
				add("Atom", new String[] { "float" }, new ParseAction() {
					public FunFloat execute(Object[] items) {
						Object value = ((Lexeme) items[0]).getValue();
						return new FunFloat((Float) value);
					}
				});
